...
- 仓库历史中出现过中文乱码问题，后续文案请尽量统一保存为 **UTF-8**
- 当前项目更像“页面编辑器原型”，不是完整低代码平台
- 目前的重点仍然是交互完整性，而不是复杂工程化能力
...
如果是在新会话中继续接着做，建议按这个顺序恢复上下文：

1. 阅读本 README
2. 查看 `src/components/RibbonBar.tsx`
3. 查看 `src/components/CanvasViewport.tsx`
4. 查看 `src/store/editorStore.ts`
5. 查看 `src/utils/exportHtml.ts`
